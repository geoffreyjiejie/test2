...
namespace GameServer.Services
{
    class ItemService : Singleton<ItemService>
    {
        ...
        void OnItemBuy(NetConnection<NetSession> sender, ItemBuyRequest request)
        {
            ...
            Log.InfoFormat("OnItemBuy: :character:{0}:Shop:{1} ShopItem:{2}", character.Id, request.shopId, request.shopItenId);
            var result = ShopManager.Instance.BuyItem(sender, request.shopId, request.shopItenId);
