...
public class TeleporterObject : MonoBehaviour {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (playerController != null && playerController.isActiveAndEnabled)
        {
            ...
            if (td == null)
            {
                Debug.LogErrorFormat("TeleObject: Charater [{0}] Enter TelePorter [{1}], But TelePorterDefine not existed", playerController.character.Info.Name, this.ID);
                return;
            }
            Debug.LogFormat("TeleObject: Charater [{0}] Enter TelePorter [{1}:{2}]", playerController.character.Info.Name, td.ID, td.Name);
            if (td.LinkTo > 0)
            {
